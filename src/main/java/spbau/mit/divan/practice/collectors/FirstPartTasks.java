package spbau.mit.divan.practice.collectors;
...
public class FirstPartTasks {
    ...
    private final static int RATING_BOUND = 95;
    ...
    public static List<Album> sortedFavorites(Stream<Album> s) {
        return s
                .filter(album -> album
                        .getTracks()
                        .stream()
                        .anyMatch(track -> track.getRating() > RATING_BOUND))
    ...
    // Сгруппировать альбомы по артистам (в качестве значения вместо объекта 'spbau.mit.divan.practice.collectors.Artist' использовать его имя)
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums
                .collect(Collectors.groupingBy(
                        Album::getName, Collectors.counting()
                ))
                .values()
                .stream()
                .collect(Collectors.summingLong(a -> a - 1));
    }
